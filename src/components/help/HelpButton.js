...
export function HelpButton () {
  ...
  const { openSidebar } = useCommonPlatform()
  ...
  return (
    <>
      <button
        ...
        type='button'
        className={`${openSidebar ? 'hidden' : ''} ${pathname.startsWith('/app/platform/chats') ? 'bottom-1 right-1' : 'bottom-4 right-4'} ${pathname.startsWith('/app/platform/chats') && userId ? 'hidden lg:inline' : ''} z-50 fixed hover:bg-gray-300 dark:hover:bg-gray-500 rounded-full bg-gray-100 dark:bg-gray-700 shadow-md text-primary-complementary dark:text-primary-dark-complementary`}
      >
